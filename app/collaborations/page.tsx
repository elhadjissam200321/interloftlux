...
import { getCollaborators, getPageContent } from '@/lib/supabase/queries'
...
export default async function CollaborationsPage() {
  const [collaborators, page] = await Promise.all([
    getCollaborators(),
  ...
  return (
    <main>
      ...
      <section className="relative w-full h-[70vh] min-h-[500px]">
        <Image
          src={content.hero_image || "https://images.unsplash.com/photo-1600210492486-724fe5c67fb0?w=1920&q=80"}
          alt={title}
          fill
      ...
      <section className="w-full py-20 md:py-28 px-8 md:px-16 lg:px-24 bg-background">
        <div className="max-w-3xl mx-auto text-center">
          <p className="label-text mb-6">{content.intro_label || "L'ART DE LA COLLABORATION"}</p>
          <p className="font-serif text-xl md:text-2xl font-light leading-relaxed text-foreground">
            {content.intro_text || "Chaque collaboration est une rencontre entre deux visions créatives. Nous croyons que le mobilier prend tout son sens lorsqu'il dialogue avec l'architecture qui l'entoure."}
          </p>
        </div>
      </section>
      ...
      <section className="w-full py-20 md:py-28 px-8 md:px-16 lg:px-24 bg-muted">
        <div className="max-w-3xl mx-auto text-center">
          ...
          <p className="text-sm md:text-base leading-relaxed text-muted-foreground mb-10 max-w-xl mx-auto">
            Architectes, designers d'intérieur — nous sommes toujours à la recherche de nouveaux talents avec qui collaborer sur des projets d'exception.
          </p>
          <Link
            href="/contact?subject=Collaboration Professionnelle&message=Bonjour, je suis un professionnel (architecte / designer) et je souhaite rejoindre votre réseau de créatifs."
